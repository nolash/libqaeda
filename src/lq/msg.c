...
//	}
	...
	*out_len += c;
	...
	r = lq_asn_write(asn, "timestamp", &timedata, c);
	if (r != ERR_OK) {
...
//		return asn_except(&item, ERR_READ);
//	}
	r = lq_asn_read(asn, "data", tmp, (int*)&c);
	if (r != ERR_OK) {
		return r;
	}
...
//		return asn_except(&item, ERR_READ);
//	}
	r = lq_asn_read(asn, "timestamp", tmp, (int*)&c);
	if (r != ERR_OK) {
		return r;
	}
	if (is_le()) {
...
//		return asn_except(&item, ERR_READ);
//	}
	r = lq_asn_read(asn, "pubkey", tmp, (int*)&c);
	if (r = ERR_OK) {
		return r;
	}
